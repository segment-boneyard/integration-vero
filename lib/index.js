...
Vero.prototype.addOrRemoveTags = function(msg, tags, fn){
  ...
  if (Object.prototype.toString.call(tags) !== '[object Object]') {
    var msg = 'Tag option must be passed as an Object.';
    return fn(new Error(msg));
  }

  if (!tags.action || (tags.action !== 'add' && tags.action !== 'remove')) {
    var msg = 'Tags action value must be either \'add\' or \'remove\'';
    return fn(new Error(msg));
  }

  if (!tags.tags || !Array.isArray(tags.tags)) {
    var msg = 'Could not detect tag values. Tag values must be passed as array of strings.';
    return fn(new Error(msg));
  }
